...
export default function MockTestsPage() {
  ...
  const [selectedSubject, setSelectedSubject] = useState("All Mock Tests");
  
  const allTests = getMockTests(params.exam_slug as string);
  
  const filteredTests = useMemo(() => {
    if (selectedSubject === "All Mock Tests") return allTests;
  ...
  return (
    <div className="space-y-6">
      <div className="mb-4">
        ...
        <p className="text-muted-foreground text-xs md:text-sm">Real exam-like environment to boost your confidence.</p>
      </div>
        ...
        <aside className="hidden lg:block w-72 shrink-0">
          <Card className="border-none shadow-md overflow-hidden sticky top-48">
            <div className="bg-primary/5 p-4 border-b">
              <span className="text-[10px] font-bold uppercase tracking-widest text-primary">Filter by Category</span>
            </div>
        ...
        <div className="flex-grow">
          <div className="mb-4 flex items-center justify-between">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">
              Showing {filteredTests.length} mock tests in {selectedSubject}
            </span>
          </div>
