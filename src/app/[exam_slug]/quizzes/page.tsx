...
import { getQuizzes, QuizItem } from '@/lib/api';
import { Card, CardContent, CardFooter } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Timer, HelpCircle, Trophy, Zap, Loader2 } from 'lucide-react';
...
export default function QuizzesPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await getQuizzes(slug);
        setQuizzes(data);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="space-y-8">
      ...
      {quizzes.length > 0 ? (
        ...
        <div className="text-center py-20 bg-muted/20 rounded-3xl border-2 border-dashed">
          ...
          <p className="text-muted-foreground">Check back later for new daily assessments.</p>
        </div>
      )}
    </div>
  );
